...
namespace Simply.Sales.TelegramBot.Infrastructure.Servicies.Message {
	public class MessageService : IMessageService {
		...
		public async Task SendPayMessage(long chatId, int orderId, OrderPrice price) {
			var labeledPrice = new List<LabeledPrice>() {
				new LabeledPrice("Цена за выбранные продукты", GetRubPrice(price.P)),
				new LabeledPrice("Скидка", price.D == 0 ? 0 : -GetRubPrice(price.D)),
				new LabeledPrice("Доставка", GetRubPrice(price.DP))
			};
		...
		/// <summary>
		/// Получение цены в рублях. По умолчанию значение у телеграмма в копейках
		/// </summary>
		private static int GetRubPrice(decimal price) =>
			(int)price * _oneRub;
	}
}
